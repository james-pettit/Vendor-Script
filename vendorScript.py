...
#defines the maximum number of seconds (in either direction) which will
#associate igs and dxf files with a pdf currently set to 30 minutes
association_window = 1800

#full paths to the various folders used to locate the files

#directory_path points to the directory crawled to find all the files--should
#point to vendor files
...
logging.basicConfig(filename="X:\Engineering\Project Folders\Project 647 - "
"Vendor Files Script\logs\log %s.txt"% time.strftime("%d-%b-%y %H%M"),
level=logging.INFO)

try:
    os.remove("X:\Engineering\Project Folders\Project 647 - "
    "Vendor Files Script\In Progress QTD list.txt")
    os.remove("X:\Engineering\Project Folders\Project 647 - "
    "Vendor Files Script\In Progress CUT list.txt")
    os.remove("X:\Engineering\Project Folders\Project 647 - "
    "Vendor Files Script\In Progress NOCUT list.txt")

except:
    print("Couldn't delete a temporary list")

try:
    qtdfile = open("X:\Engineering\Project Folders\Project 647 - "
    "Vendor Files Script\In Progress QTD list.txt", 'a')
    cutfile = open("X:\Engineering\Project Folders\Project 647 - "
    "Vendor Files Script\In Progress CUT list.txt", 'a')
    nocutfile = open("X:\Engineering\Project Folders\Project 647 - "
    "Vendor Files Script\In Progress NOCUT list.txt", 'a')
...
#Converts the entire pdf to a string of text, and returns this text. Needs to be
#passed a full path
...
def convert_pdf_to_txt(path):
    ...
    caching = True
    pagenos=set()
    for page in PDFPage.get_pages(fp, pagenos, maxpages=maxpages, password="",
...
def find_filename(filepath):
    ...
    if re.search("\d\d\d\d\d\d [RrXx]-\d\d\.",filepath) is not None:
        ...
        try:
            ...
            if re.search("THIS PART DOES NOT USE A CUT", pdftext) is not None:
                print("NOCUT")
...
#Returns rev information as a string of undermined length if passed a valid file
#ending in ".pdf". Returns None if passed an invalid filename and explains that
#the file doesn't exist. Silently returns None if passed a file that doesn't end
#in ".pdf"
...
#when passed a directory path, lists all the files in the folder, and all
...
#renames the first filepath called to the second filepath called. If the
#filepaths are the same, no change is made if the second filepath already exists
#the files are both hashed, if they are the same, the original_filepath file is
#deleted. if the hashes don't match, the filename will be modified until it
...
for file_to_open in file_list:
    ...
    "[RrXx]\d+\.|\d\d\d\d\d\d REV.+\.)",
    os.path.basename(file_to_open)) is not None:
        #file has the correct name format, if it's a pdf, it can be renamed now,
        #and it's rev added to revlist
        gracefulRename(file_to_open, os.path.join(directory_path,
        find_filename(file_to_open)))
...
try:
    os.remove("X:\Engineering\Project Folders\Project 647 - Vendor Files Script"
    "\QTD list.txt")
    os.remove("X:\Engineering\Project Folders\Project 647 - Vendor Files Script"
    "\CUT list.txt")
    os.remove("X:\Engineering\Project Folders\Project 647 - Vendor Files Script"
...
os.rename("X:\Engineering\Project Folders\Project 647 - Vendor Files Script"
"\In Progress QTD list.txt","X:\Engineering\Project Folders"
"\Project 647 - Vendor Files Script\QTD list.txt")
os.rename("X:\Engineering\Project Folders\Project 647 - Vendor Files Script"
"\In Progress CUT list.txt","X:\Engineering\Project Folders"
"\Project 647 - Vendor Files Script\CUT list.txt")
os.rename("X:\Engineering\Project Folders\Project 647 - Vendor Files Script"
"\In Progress NOCUT list.txt","X:\Engineering\Project Folders"
"\Project 647 - Vendor Files Script\NOCUT list.txt")
...
for file_to_open in file_list:
    ...
    os.path.basename(new_filepath)) is not None:

        #move old revs to archive here
        rev_info = re.search("(\d\d\d\d\d\d).+(\..+)",
        ...
        if number_of_revs > 1:
            for rev in rev_list[number][1:]:
                gracefulRename(os.path.join
                (os.path.dirname(new_filepath), number + ' ' + rev + extension),
